"""
9_🎙️_The_Studio — Joseph M. Smith AI Analyst Broadcast Desk.

Full dedicated page for Joseph's live analysis desk: player lookups,
game strategy breakdowns, best bet generation, ticket building,
and track record display.
"""
...
st.set_page_config(page_title="The Studio — Joseph M. Smith", page_icon="🎙️", layout="wide")

# ── Imports (graceful fallbacks) ────────────────────────────────────────

try:
    from SmartPicksProAI.styles.theme import get_global_css  # noqa: E402
    st.markdown(get_global_css(), unsafe_allow_html=True)
except ImportError:
    pass

try:
    from pages.helpers.joseph_live_desk import (  # noqa: E402
...
try:
    from SmartPicksProAI.engine.joseph_brain import (  # noqa: E402
...
try:
    from SmartPicksProAI.engine.joseph_bets import (  # noqa: E402
...
try:
    from SmartPicksProAI.engine.joseph_tickets import (  # noqa: E402
        build_joseph_ticket,
    )
...
with tab_analysis:
    ...
    if not _API_AVAILABLE:
        ...
        with col_input:
            player_query = st.text_input("Search Player", placeholder="e.g. LeBron James")
            stat_type = st.selectbox(
                "Stat Type",
                ["points", "rebounds", "assists", "threes", "steals", "blocks",
                 "points_rebounds_assists", "points_rebounds", "points_assists",
                 "rebounds_assists", "fantasy_score_pp"],
            )
            prop_line = st.number_input("Prop Line", min_value=0.0, step=0.5, value=20.0)
            ...
            if run_btn and player_query:
                with st.spinner("Joseph is breaking this down..."):
                    ...
                    if not players:
                        ...
                        player_name = player.get("full_name", player.get("player_name", player_query))

                        # Run prop analysis via API
                        result = analyze_prop(
...
with tab_best_bets:
    ...
    if not _BRAIN_AVAILABLE:
        st.info("Joseph's brain module is not available. Ensure all engine modules are installed.")
    else:
        st.caption(
...
with tab_tickets:
    ...
    if not _TICKETS_AVAILABLE:
        ...
        if slate_results:
            ...
            min_conf = st.slider("Min Confidence", 40.0, 90.0, 60.0, step=5.0)

            if st.button("🎫 Build Ticket", type="primary"):
                with st.spinner("Building ticket..."):
                    ticket = build_joseph_ticket(
                        slate_results, max_legs=max_legs, min_confidence=min_conf,
                    )
        ...
        if total > 0:
            ...
            if by_tier:
                ...
                for tier_name, tier_data in by_tier.items():
                    if isinstance(tier_data, dict):
                        ...
                        st.markdown(f"- **{tier_name}**: {t_wins}/{t_total} ({t_rate:.0f}%)")
            ...
            if recent:
                ...
                for bet in recent[:20]:
                    result_emoji = {"win": "✅", "loss": "❌", "push": "➖"}.get(
                        bet.get("result", ""), "⏳"
                    )
                    st.markdown(
                        f"{result_emoji} {bet.get('player_name', '?')} — "
                        f"{bet.get('stat_type', '?')} {bet.get('direction', '?')} "
                        f"{bet.get('prop_line', '?')} ({bet.get('confidence_tier', '')})"
                    )
        else:
